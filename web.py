...
@app.route("/blackout", methods=["GET"])
@require_token
def blackout():
    async def update():
        display.grid.clear()
...
def setpixel():
    ...
    async def update():
        ...
        if data['data_type'] == 'single':
            ...
            display.grid[data['data']['x']][data['data']['y']] = tuple(data['data']['color'])
            ...
            try:
                for x, y_data in data['data'].items():
                    for y, color in y_data.items():
                        display.grid[int(x)][int(y)] = tuple(color)
...
def fill_region():
    ...
    async def update():
        if request.args.get('clear') == "true":
            display.grid.clear()
        display.grid.fill_rect(data['a_coords'][0], data['a_coords'][1], data['b_coords'][0], data['b_coords'][1], data['color'])
        if request.args.get('update') == "true":
            await display.send_grid()
        return {"success": True}
...
@app.route("/multitool", methods=["POST"])
@require_token
def multitool():
    ...
    async def update():
        def _setpixel(data_item):
            if data_item['data_type'] == 'single':
                # Dict looks like: {'data_type': 'single', 'data': {'x': 10, 'y': 15, 'color': [255, 128, 64]}}
                display.grid[data_item['data']['x']][data_item['data']['y']] = tuple(data_item['data']['color'])
        ...
        def _fill(data_item):
            display.grid.fill_rect(data_item['a_coords'][0], data_item['a_coords'][1], data_item['b_coords'][0], data_item['b_coords'][1], data_item['color'])
            return {"success": True}
